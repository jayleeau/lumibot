...
The frozen HTS timing and execution contract is
[`docs/investigations/2026-09-15_HOURLY_CONVENTION.md`](investigations/2026-09-15_HOURLY_CONVENTION.md).
...
| P1 | **Regular-session cleanliness.** Clock-hour bar labelling is closed by the frozen `09:00-15:00` / available-at contract; strict RTH cleanliness is separate. Native LumiBot prevents lookahead structurally, but it cannot tell whether a source hour is clean. | Verify premarket contamination, early closes, and DST. An hour containing premarket trades cannot be cleaned by relabelling it. |
...
- **Timing:** use the frozen clock-hour/availability rule: labels `09:00` through `15:00` ET, interval `[T, T+1h)`, and availability at `T+1h`. The `14:00` close is actionable at `15:00`; the `15:00` close can only queue the next-session open. Stops evaluate at completed hourly bars and manifests record literal timestamps.
